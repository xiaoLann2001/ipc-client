...
void VideoOpenGL::setImage(char *data_ptr)
{
...
void VideoOpenGL::setImage(const QImage &image)
{
    ...
    case QImage::Format_RGB888:
        ...
        break;
    default:
        qDebug() << "Unknow image format.";
        return;
    }

    // 设置图像大小
    this->setImageSize(image.width(), image.height(), (format));

    memcpy(this->m->m_video_ptr, image.bits(), this->m->m_image_size);
    update();
}